...
async function favoriteEventHandler(event) {
  const favorites = getFavorites();
...
async function favoritesHandler(buttons) {
...
async function windowActions() {
  ...
  //localStorage.removeItem('diningFavorites');
  // setFavorites(['empty']);
  const allDiningHalls = await getDiningHallData();
  const favorites = getFavorites();
  ...
  populateDiningHalls(allDiningHalls, favoritesArr);

  const buttons = document.querySelectorAll('.favorite');
  await favoritesHandler(buttons);
}
